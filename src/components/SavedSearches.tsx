...
import { saveSearch, deleteSavedSearch } from "@/app/(app)/animals/favourites-actions";

// Save the current animals view (its querystring) under a name, and jump back to
// any saved view. Purely per-user; the list is passed in from the server.
export default function SavedSearches({
  ...
  const save = () => {
    ...
    if (!nm) return;
    start(async () => {
      await saveSearch(nm, path, currentQuery);
      setName("");
      setNaming(false);
      router.refresh();
    });
      ...
      {searches.map((s) => (
        <span key={s.id} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-700">
          ...
          <button
            ...
            title="Delete"
            disabled={pending}
            onClick={() => start(async () => { await deleteSavedSearch(s.id); router.refresh(); })}
