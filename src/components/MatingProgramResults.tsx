 ...
 */
function bestIndexOf(f: MatingFemale): number | null {
...
export function MatingProgramResults({
  ...
  return (
    <div>
      ...
      {view === "herd" ? (
        <HerdSummary report={report} indexLabel={indexLabel} />
      ) : (
...
function HerdSummary({ report, indexLabel }: { report: MatingReport; indexLabel: string }) {
  return (
      ...
      <Table
        head={
          <>
            ...
            <th className="th">Top 3 bulls — projected calf {indexLabel}</th>
                ...
                <ol className="space-y-0.5 text-xs text-slate-700">
                  {f.matches.slice(0, 3).map((m, r) => (
                    <li key={m.bullId}>
                      ...
                      {damLacksIndex(f) && <span className="text-slate-400"> (bull&rsquo;s own)</span>}
                    </li>
                  ))}
                </ol>
              )}
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
          ...
          {best !== null && (
            <span className="text-sm font-semibold text-brand-700">
              {fallback ? `Best bull ${indexLabel}` : `Best calf ${indexLabel}`} {fmtNum(best)}
            </span>
          )}
            ...
            <>
              ...
              {fallback && (
                <p className="mb-3 rounded border border-amber-200 bg-amber-50 px-2 py-1.5 text-xs text-amber-800">
                  ...
                  falls back to each bull&rsquo;s own {indexLabel} — it is <strong>not</strong> a projected calf value.
                </p>
              )}
              ...
              {f.matches.length === 0 ? (
                ...
                >
                  {f.matches.map((m, i) => (
                    <MatchRow key={m.bullId} match={m} rank={i + 1} fallback={fallback} />
                  ))}
                </Table>
              )}
...
function MatchRow({ match: m, rank, fallback }: { match: MatingMatch; rank: number; fallback: boolean }) {
