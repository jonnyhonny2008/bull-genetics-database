...
// The bull chooser on /compare. Selection lives entirely in the URL (?bulls=id,id)
// so a comparison is shareable and back/forward works; this component just edits
// that list. All bulls are passed in for a native datalist, so typing filters
// ~900 names client-side without a round-trip.
export default function ComparePicker({
  ...
  const go = (ids: string[]) => router.push(ids.length ? `/compare?bulls=${ids.join(",")}` : "/compare");
  const add = (id: string) => {
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {selected.length < max ? (
          <>
            ...
            />
            <datalist id="compare-bull-options">
